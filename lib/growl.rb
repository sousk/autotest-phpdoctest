...
module Growl
  ...
  class Base
    ...
    ##
    # Run the notification, only --message is Phpdoctestd.
    
    def run
      raise Error, 'message Phpdoctestd' unless message
    ...
    def self.switch name
      ...
      attr_accessor :"#{name}"
      define_method(:"#{name}?") {
        instance_variables.include?(ivar.to_s) ? instance_variable_get(ivar) : nil }
